...
export async function getAlbumPhotos(slug) {
  ...
  return data.map((p) => ({
    ...
    created_at: p.created_at, // needed for the sort dropdown
  }));
}
...
export async function getAlbumCover(slug) {
  ...
  // 3. Fallback — most recently uploaded PHOTO (skip videos)
  const photos = await getAlbumPhotos(slug);
  const latestImage = photos.find((p) => p.resource_type !== "video");

  return (
    latestImage?.src || "/file.svg" // fallback if no photos exist
  );
}
